...
1. add a file input field called "contact\_image\_input" `<input type="file" name="contact_image_input"/>`
...
Then, let's add it to the two methods. Add it _after_ `isLoggedIn` \(no need to process the image if the user isn't logged in\). Don't forget to change `get`s to `post`s!
